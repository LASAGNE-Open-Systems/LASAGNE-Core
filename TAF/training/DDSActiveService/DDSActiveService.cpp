...
#define LTM_DDSACTIVESERVICE_CPP

// Include our DDSActiveService declaration
...
ACE_FACTORY_DEFINE(DDSActiveService, LTM_DDSActiveService); // Define the dynamic deployment factory
ACE_STATIC_SVC_DEFINE(LTM_DDSActiveService              // Define the static deployment descriptor
    , LTM_DDSActiveService::svc_ident()                 // Define the container ident key
    , ACE_SVC_OBJ_T                                     // Define it to be base on ACE_Service_Object
    , &ACE_SVC_NAME(LTM_DDSActiveService)               // Define the dynamic factory address
...
namespace LTM  // Open the LTM Namespace
{
    ...
    DDSActiveService::DDSActiveService(void) : DAF::TaskExecutor() // Often decorate with call to base
        , debug_        (0)
    ...
    {
        ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::DDSActiveService(void)\n"), this));
    }

    // Destructor Implementation - Cleanup resources - Used by deployment factories
    DDSActiveService::~DDSActiveService(void)
    {
        ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::~DDSActiveService(void)\n"), this));
    ...
    int
    DDSActiveService::parse_args(int argc, ACE_TCHAR *argv[])
    {
        ACE_Get_Opt get_opts(argc, argv, ACE_TEXT("f:t:p:d:z::"), 0); // NOTE: starting from argv[0]
            ...
            case '?': default:  // Unknown argument
                if (this->debug_) {
                    ACE_DEBUG((LM_WARNING, ACE_TEXT("(%P | %t) %s encountered unknown argument #%d\n")
                        ...
                        , LTM_DDSActiveService::svc_ident(), arg_number));
                }
                break;
            }
        }
    ...
    int
    DDSActiveService::init(int argc, ACE_TCHAR *argv[])
    {
        ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::init(int argc, ACE_TCHAR *argv[])\n")
    ...
    int
    DDSActiveService::suspend(void)
    {
        if (this->debug()) {
            ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::suspend(void).\n"), this));
        }
    ...
    int
    DDSActiveService::resume(void)
    {
        if (this->debug()) {
            ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::resume(void).\n"), this));
        }
    ...
    int
    DDSActiveService::fini(void)
    {
        ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::fini(void)\n"), this));
    ...
    int
    DDSActiveService::info(ACE_TCHAR **info_string, size_t length) const
    {
        ...
        ACE_DEBUG((LM_INFO, ACE_TEXT("(%P | %t) LTM::DDSActiveService[%@]::info(ACE_TCHAR **info_string, size_t length=%d)\n")
    ...
    int
    DDSActiveService::handle_timeout(const ACE_Time_Value &expected_time, const void *act)
    {
        ACE_UNUSED_ARG(act);
    ...
    DDS::ReturnCode_t
    DDSActiveService::on_data_available(const LTMTopicDetailsTopicType &td)
    {
        if (this->suspended_) {
    ...
    int
    DDSActiveService::svc_reader(int index)
    {
        if (this->debug()) {
    ...
    int
    DDSActiveService::svc_writer(int index)
    {
        if (this->debug()) {
        ...
        DDSActivePublisher      dcpsPublisher;
        ...
        if (this->isAvailable()) {
            ...
            if (dcpsPublisher.init(this->dcpsParticipant_) != DDS::RETCODE_OK) {
                ACE_ERROR_RETURN((LM_ERROR,
                    ...
                    , LTM_DDSActiveService::svc_ident(), ltm::LTMTopicDetailsName), -1);
            }
